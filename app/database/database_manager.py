...
def search_products(mysql, query):
    if not mysql:
        return []
    ...
    try:
        cursor = mysql.get_db().cursor()
        query = query.replace("-","")
        query = "SELECT * from (PRODUCT LEFT JOIN CATEGORY ON PRODUCT.category_id = CATEGORY.id) LEFT JOIN furniture.CONDITION ON PRODUCT.condition_id = furniture.CONDITION.id where PRODUCT.name LIKE '%" + query + "%';"
        print(query)
